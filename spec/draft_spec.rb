...
describe NylasDashboardV2SDK::Draft do
  before (:each) do
    @app_id = 'ABC'
    @app_secret = '123'
  ...
  describe "#save!" do
    it "does save all the fields of the draft object and only sends the required JSON" do
      ...
      draft = NylasDashboardV2SDK::Draft.new(@inbox)
  ...
  describe "#send!" do
    ...
    error_codes = NylasDashboardV2SDK::HTTP_CODE_TO_EXCEPTIONS.to_a
    ...
    it "sets server_error to nil when it isn't defined" do
      ...
      expect { draft.send! }.to raise_error(NylasDashboardV2SDK::InvalidRequest)
      begin
        draft.send!
      rescue NylasDashboardV2SDK::InvalidRequest => e
        expect(e.message).to eq("Invalid recipient address benbitdiddle@gmailcom")

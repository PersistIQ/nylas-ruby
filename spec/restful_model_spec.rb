...
describe 'RestfulModel' do
  before (:each) do
    @api = NylasDashboardV2SDK::API.new('app_id', 'app_secret', 'key')
  end

  describe "#as_json" do
    it "should return a hash with the parameter properties" do
      r = NylasDashboardV2SDK::RestfulModel.new(@api)
      r.id = '1'
      r.account_id = '123';
    ...
    it "should ignore arbitrary setters" do
      model_subclass = Class.new(NylasDashboardV2SDK::RestfulModel) do
        def foo=(bar); end
      end
